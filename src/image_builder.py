...
def image_builder(buildspec, image_types=[], device_types=[], profile=None):
    """
    ...
    if (
        ...
        os.system(
            f"aws ecr get-login-password --region us-west-2 {f'--profile {profile}' if profile else ''}  | docker login --username AWS --password-stdin 763104351884.dkr.ecr.us-west-2.amazonaws.com"
        )
    ...
    for image_name, image_config in BUILDSPEC["images"].items():
        ...
        if label_device_type == "gpu":
            label_device_type = (
                f"{label_device_type}.{str(image_config['cuda_version'])}"
            )
        ...
        if cx_type == "sagemaker":
            ...
            labels[
                f"com.amazonaws.ml.engines.{cx_type}.dlc.os.{label_os_version}"
            ] = "true"

            labels[
                f"com.amazonaws.ml.engines.{cx_type}.dlc.job.{label_job_type}"
            ] = "true"
    ...
    if BUILT_IMAGES:
        ...
        upload_metrics(
            BUILT_IMAGES, BUILDSPEC, is_any_build_failed, is_any_build_failed_size_limit
        )
...
def generate_common_stage_image_object(pre_push_stage_image_object, image_tag):
    ...
    pre_push_stage_image_object.corresponding_common_stage_image = (
        common_stage_image_object
    )
...
def upload_metrics(
    images, BUILDSPEC, is_any_build_failed, is_any_build_failed_size_limit
):
    """
